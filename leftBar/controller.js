  ...
  for (var i=0, file; file=files[i]; i++) {
  	...
  	if (images.indexOf(extension) !== -1) {
  		...
  		if(e.srcElement.nodeName == 'SPAN') {
  			...
  			ipcRenderer.send('change-image', file.path, "left", e.srcElement.parentNode.id)
  		}
  		else {
		  ...
		  .then(stream => {
		    ...
		    mediaRecorder.addEventListener("stop", (event) => {
		      ...
		      const audioUrl = URL.createObjectURL(audioBlob);
		 			...
		 			ipcRenderer.send('show-prompt', pressed.src, "left", drop)
...
ipcRenderer.on('update-keymap', (event, tempKeyMap) => {
	for (var [id, code] of keyMap) {
		for(var [tempID, tempCode] of tempKeyMap) {
	    	...
	    	oldSpan.data = null
	    	keyMap.delete(id)
	    	ipcRenderer.send('clean-keyMap', "left", JSON.stringify([...keyMap]))
	    	break
	    }
		}
	}
